 ...
 * 3. View similar/related products
 * 4. Navigate to a related product
 * 5. Add related product to cart
 * 6. Product with no similar products
 */

test.describe('Product Details — product-details.e2e.js', () => {
  let PRODUCT_URL;
  let PRODUCT_SLUG;

  test.beforeAll(async ({ request }) => {
    const response = await request.get(`${process.env.REACT_APP_API}/api/v1/product/get-product`);
    const data = await response.json();
    const firstProduct = data.products[0];
    PRODUCT_SLUG = firstProduct.slug;
    PRODUCT_URL = `${process.env.REACT_APP_CLIENT}/product/${PRODUCT_SLUG}`;
  });

  test.beforeEach(async ({ page }) => {
    await page.goto(PRODUCT_URL);
    await expect(page.getByTestId('product-title')).not.toHaveText('', { timeout: 15000 });
  });

  test('1 View product details', async ({ page }) => {
  ...
  test('2 Add to cart from product detail page', async ({ page }) => {
...
test('3 View similar/related products', async ({ page }) => {
...
test('4 Navigate to a related product', async ({ page }) => {
...
test('5 Add related product to cart', async ({ page }) => {
	const similarList = page.getByTestId('similar-products-list');
	await expect(similarList.locator('.card').first()).toBeVisible({ timeout: 10000 });
  ...
  test('6 Product with no similar products', async ({ page }) => {
		await page.goto(`${process.env.REACT_APP_CLIENT}/product/nus-tshirt`);
		await expect(page.getByTestId('product-title')).toBeVisible({ timeout: 10000 });
